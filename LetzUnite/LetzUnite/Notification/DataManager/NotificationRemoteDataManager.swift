...
class NotificationRemoteDataManager: NotificationRemoteDataManagerInputProtocol {
    var remoteRequestHandler: NotificationRemoteDataManagerOutputProtocol?
    var validationManager: NotificationValidationInputProtocol?
    ...
    func callApi(with endpoint:Endpoint, parameters:Dictionary<String,Any>, method: RequestMethod) {
        ...
        loginOp.execute(in: service, retry: 0).then { (res) -> (Void) in
            ...
            if let resp = res {
                switch endpoint {
                case .reward:
                    self.remoteRequestHandler?.onFetchNotification(resp as? RewardResponse)
                default:
                    print("default")
                }
            }else {
...
extension NotificationRemoteDataManager: NotificationValidationOutputProtocol {
    func didValidate(_ parameters: Dictionary<String, Any>, endpoint: Endpoint) {
        switch endpoint {
        case .reward:
            self.callFetchNotificationApi(With: parameters, endpoint: endpoint)
        default:
            print("default")
        }
    }
