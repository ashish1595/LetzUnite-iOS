//
//  FeedsDetailRemoteDataManager.swift
//  LetzUnite
//
...
class FeedsDetailRemoteDataManager: FeedsDetailRemoteDataManagerInputProtocol {
    var remoteRequestHandler: FeedsDetailRemoteDataManagerOutputProtocol?
    var validationManager: FeedsDetailValidationInputProtocol?
    ...
    func callApi(with endpoint:Endpoint, parameters:Dictionary<String,Any>, method: RequestMethod) {
        ...
        loginOp.execute(in: service, retry: 0).then { (res) -> (Void) in
            ...
            if let resp = res {
                switch endpoint {
                case .reward:
                    self.remoteRequestHandler?.onFetchFeedsDetail(resp as? RewardResponse)
                default:
                    print("default")
                }
            }else {
...
extension FeedsDetailRemoteDataManager: FeedsDetailValidationOutputProtocol {
    func didValidate(_ parameters: Dictionary<String, Any>, endpoint: Endpoint) {
        switch endpoint {
        case .reward:
            self.callFetchFeedsDetailApi(With: parameters, endpoint: endpoint)
        default:
            print("default")
        }
    }
