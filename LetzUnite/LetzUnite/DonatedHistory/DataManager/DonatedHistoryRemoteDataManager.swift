//
//  DonatedHistoryRemoteDataManager.swift
//  LetzUnite
//
...
class DonatedHistoryRemoteDataManager: DonatedHistoryRemoteDataManagerInputProtocol {
    var remoteRequestHandler: DonatedHistoryRemoteDataManagerOutputProtocol?
    var validationManager: DonatedHistoryValidationInputProtocol?
    ...
    func callApi(with endpoint:Endpoint, parameters:Dictionary<String,Any>, method: RequestMethod) {
        ...
        loginOp.execute(in: service, retry: 0).then { (res) -> (Void) in
            ...
            if let resp = res {
                switch endpoint {
                case .reward:
                    self.remoteRequestHandler?.onFetchDonatedHistory(resp as? RewardResponse)
                default:
                    print("default")
                }
            }else {
...
extension DonatedHistoryRemoteDataManager: DonatedHistoryValidationOutputProtocol {
    func didValidate(_ parameters: Dictionary<String, Any>, endpoint: Endpoint) {
        switch endpoint {
        case .reward:
            self.callFetchDonatedHistoryApi(With: parameters, endpoint: endpoint)
        default:
            print("default")
        }
    }
