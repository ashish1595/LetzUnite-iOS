...
//  LetzUnite
//
//  Created by Himanshu on 4/28/18.
...
class RSAWrapper {
    ...
    func getPublicKeyFromBundle(fileName:String, extensionType:String) -> Bool {
        
        if let fileUrl = Bundle.main.url(forResource: fileName, withExtension: extensionType) {
                do {
                    let certificateData = try Data(contentsOf: fileUrl as URL) as CFData
