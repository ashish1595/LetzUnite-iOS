...
+ (BOOL)processResult:(CCCryptorStatus)result 
  ...
  if (result != kCCSuccess) {
    ...
    //DLog(@"Could not process data: %d", result);
    return NO;
  }
...
+ (NSDictionary *)encryptedDataForDict:(NSData *)data
                        password:(NSString *)password
                              iv:(NSData **)iv
                            salt:(NSData **)salt
                           error:(NSError **)error {
    ...
    *salt = [self randomDataOfLength:kPBKDFSaltSize];
    ...
    NSDictionary * dict = [[NSDictionary alloc]initWithObjectsAndKeys:cipherData,@"encryptedData",(*iv),@"ivData",(*salt),@"saltData", nil];
