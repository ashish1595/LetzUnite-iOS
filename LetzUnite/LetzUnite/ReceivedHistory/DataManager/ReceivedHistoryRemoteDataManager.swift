//
//  ReceivedHistoryRemoteDataManager.swift
//  LetzUnite
//
...
class ReceivedHistoryRemoteDataManager: ReceivedHistoryRemoteDataManagerInputProtocol {
    var remoteRequestHandler: ReceivedHistoryRemoteDataManagerOutputProtocol?
    var validationManager: ReceivedHistoryValidationInputProtocol?
    ...
    func callApi(with endpoint:Endpoint, parameters:Dictionary<String,Any>, method: RequestMethod) {
        ...
        loginOp.execute(in: service, retry: 0).then { (res) -> (Void) in
            ...
            if let resp = res {
                switch endpoint {
                case .reward:
                    self.remoteRequestHandler?.onFetchReceivedHistory(resp as? RewardResponse)
                default:
                    print("default")
                }
            }else {
...
extension ReceivedHistoryRemoteDataManager: ReceivedHistoryValidationOutputProtocol {
    func didValidate(_ parameters: Dictionary<String, Any>, endpoint: Endpoint) {
        switch endpoint {
        case .reward:
            self.callFetchReceivedHistoryApi(With: parameters, endpoint: endpoint)
        default:
            print("default")
        }
    }
